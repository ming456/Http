...
public class JF_view_query_kcmx_ck extends JInternalFrame {
    /**
     * 
     */
    private static final long serialVersionUID = 8607879631190398594L;
    ...
    String[] name = { "xsdjh", "xssl", "xsje" };
    int _idIndex, _ysfIndex;
    
    public JF_view_query_kcmx_ck() {
        _idIndex = 0;
        _ysfIndex = 0;
        try {
            buildTable();
    ...
    private void jbInit() throws Exception {
        ...
        jCheckBox2.setText("销售日期");
        jCheckBox2.addChangeListener(new JF_view_query_kcmx_ck_jCheckBox2_changeAdapter(this));
        ...
        jCid.addItemListener(new JF_view_query_kcmx_ck_jCid_itemAdapter(this));
        jPanel2.setBorder(border1);
        jPanel3.setBorder(border2);
        jTvalue.setPreferredSize(new Dimension(156, 24));
        jCysf.setPreferredSize(new Dimension(53, 23));
        jCysf.addItemListener(new JF_view_query_kcmx_ck_jCysf_itemAdapter(this));
        jByes.setFont(new java.awt.Font("新宋体", Font.PLAIN, 12));
        jByes.setText("查询");
        jByes.addActionListener(new JF_view_query_kcmx_ck_jByes_actionAdapter(this));
    ...
    public void jByes_actionPerformed(ActionEvent e) {
        ...
        tModel = new DefaultTableModel(vname, 0);
        ...
        if (jCheckBox1.isSelected() && !jCheckBox2.isSelected()) {
            ...
            if (_ysfIndex == 0) {
                whereStr = name[_idIndex] + " " + jCysf.getSelectedItem() + " '%" + jTvalue.getText().trim() + "%' and xsrq >= '" + date1 + "' and xsrq <= '"
                        + date2 + "'";
            } else {
                whereStr = name[_idIndex] + " " + jCysf.getSelectedItem() + " '" + jTvalue.getText().trim() + "' and xsrq >= '" + date1 + "' and xsrq <= '"
                        + date2 + "'";
            ...
            whereStr = " xsrq >= '" + date1 + "' and xsrq <= '" + date2 + "'";
            
        } else if (!jCheckBox1.isSelected() && !jCheckBox2.isSelected()) {
            return;
        }
        queryStr = " from Obj_SellDetaile where " + whereStr + "and thsts='F'";
        ...
        while (iterator.hasNext()) {
            Obj_SellDetaile buy = (Obj_SellDetaile) iterator.next();
            java.util.Vector<Comparable> vdata = new java.util.Vector<Comparable>();
            vdata.addElement(buy.getXsdjh());
            vdata.addElement(buy.getYpbh());
            vdata.addElement(String.valueOf(buy.getXsdj()));
            vdata.addElement(String.valueOf(buy.getXssl()));
            vdata.addElement(String.valueOf(buy.getXsje()));
            vdata.addElement(buy.getXsrq());
...
class JF_view_query_kcmx_ck_jByes_actionAdapter implements ActionListener {
    private JF_view_query_kcmx_ck adaptee;
    
    JF_view_query_kcmx_ck_jByes_actionAdapter(JF_view_query_kcmx_ck adaptee) {
...
class JF_view_query_kcmx_ck_jCid_itemAdapter implements ItemListener {
    private JF_view_query_kcmx_ck adaptee;
    
    JF_view_query_kcmx_ck_jCid_itemAdapter(JF_view_query_kcmx_ck adaptee) {
        this.adaptee = adaptee;
    }
...
class JF_view_query_kcmx_ck_jCysf_itemAdapter implements ItemListener {
    private JF_view_query_kcmx_ck adaptee;
    
    JF_view_query_kcmx_ck_jCysf_itemAdapter(JF_view_query_kcmx_ck adaptee) {
        this.adaptee = adaptee;
    }
...
class JF_view_query_kcmx_ck_jCheckBox2_changeAdapter implements ChangeListener {
    private JF_view_query_kcmx_ck adaptee;
    
    JF_view_query_kcmx_ck_jCheckBox2_changeAdapter(JF_view_query_kcmx_ck adaptee) {
        this.adaptee = adaptee;
    }
