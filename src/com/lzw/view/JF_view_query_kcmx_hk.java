...
public class JF_view_query_kcmx_hk extends JInternalFrame {
     ...
     * 
     */
    private static final long serialVersionUID = -8728371571115016532L;
    ...
    public void jByes_actionPerformed(ActionEvent e) {
        ...
        if (jCheckBox1.isSelected() && !jCheckBox2.isSelected()) {
            ...
            if (_ysfIndex == 0) {
                whereStr = name[_idIndex] + " " + jCysf.getSelectedItem() + " '" + jTvalue.getText().trim() + "%' and hkrq >= '" + date1 + "' and hkrq <= '"
                        + date2 + "'";
            } else {
                whereStr = name[_idIndex] + " " + jCysf.getSelectedItem() + " '" + jTvalue.getText().trim() + "' and hkrq >= '" + date1 + "' and hkrq <= '"
                        + date2 + "'";
